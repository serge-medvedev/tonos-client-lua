describe("an abi test suite #abi", function()
    local context = require "ton.context"
    local abi= require "ton.abi"
    local crypto = require "ton.crypto"
    local json = require "dkjson"
    local tt = require "test.tools"

    local ctx

    setup(function()
        local config = '{"network": {"server_address": "https://net.ton.dev"}}'

        ctx = context.create(config)
    end)

    teardown(function()
        context.destroy(ctx)
    end)

    describe("an abi.decode_message", function()
        it("should return decoded data", function()
            local encoded = tt:create_encoded_message(ctx, { WithKeys = tt.keys }).await()
            local result = abi.decode_message(
                ctx, { Serialized = json.decode(tt.events.abi) }, encoded.message).await()

            assert.equals("constructor", result.name)
        end)
    end)

    describe("an abi.attach_signature", function()
        it("should return a signed message", function()
            local encoded = tt:create_encoded_message(
                ctx, { External = tt.keys.public }, 1599458364291, 1599458404).await()
            local signature = crypto.nacl_sign_detached(
                ctx, encoded.message, tt.keys.public .. tt.keys.secret).await().signature
            local result = abi.attach_signature(
                ctx,
                { Serialized = json.decode(tt.events.abi) },
                tt.keys.public,
                encoded.message,
                signature).await()

            assert.equals("", result.message)
        end)
    end)

    describe("an abi.encode_message", function()
        it("should return a BOC", function()
            local result = abi.encode_message(
                ctx,
                { Serialized = json.decode(tt.events.abi) },
                nil,
                { tvc = tt.events.tvc },
                { function_name = "constructor", header = { pubkey = tt.keys.public } },
                { WithKeys = tt.keys }).await()

            assert.is_not_nil(result.address)
            assert.is_not_nil(result.message)
        end)
    end)

    describe("an abi.encode_account", function()
        it("should return encoded account data", function()
            local elector_encoded = abi.encode_account(
                ctx, { StateInit = { code = tt.elector.code, data = tt.elector.data } }).await()

            assert.equals("1089829edf8ad38e474ce9e93123b3281e52c3faff0214293cbb5981ee7b3092", elector_encoded.id)
        end)
    end)
end)

